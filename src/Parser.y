...
%type <OpenABL::AST::Declaration *> declaration func_decl agent_decl const_decl env_decl struct_decl;
...
agent_decl: AGENT IDENTIFIER LBRACE agent_member_list RBRACE
              { $$ = new AgentDeclaration($2, true, $4, @$); };

struct_decl: IDENTIFIER LBRACE agent_member_list RBRACE
              { $$ = new AgentDeclaration($1, false, $3, @$); };
...
agent_member: opt_position type IDENTIFIER SEMI { $$ = new AgentMember($1, false, $2, $3, @$); };
            | opt_position type IDENTIFIER LBRACKET literal RBRACKET SEMI { $$ = new AgentMember($1, true, $2, $3, @$); };
...
ident_list: IDENTIFIER { $$ = new IdentList(); $$->push_back($1); }
          ...
          | IDENTIFIER LPAREN INT RPAREN { $$ = new IdentList(); $$->push_back($1);  $$->push_back( std::to_string($3));};
          | ident_list COMMA IDENTIFIER LPAREN INT RPAREN { $1->push_back($3);$1->push_back(std::to_string($5)); $$ = $1; };
...
statement: expression SEMI { $$ = new ExpressionStatement($1, @$); }
         ...
         | CONFLICTRESOLVER LBRACE expression COMMA expression COMMA ident_list optional_comma RBRACE
             { $$ = new ConflictResolutionStatement($3, $5, $7, @$); }
...
expression: var { $$ = new VarExpression($1, @$); }
          ...
          | type var LBRACKET expression RBRACKET { $$ = new ArrayCreationExpression($2, $1, $4, @$); }
