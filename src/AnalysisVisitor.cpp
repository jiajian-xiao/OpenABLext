...
void AnalysisVisitor::leave(AST::SimulateStatement &stmt) {
  ...
  for (const std::string &name : *stmt.stepFuncs) {
    if (isdigit(name[0]))
    {
        int deviceId = std::stoi( name );
        if (stmt.stepFuncDeclOnDevice[deviceId].size()<1) {
            stmt.num_devices++;
            stmt.activeDevices.push_back(deviceId);
        }
        AST::FunctionDeclaration* fn = stmt.stepFuncDecls.back();
        stmt.stepFuncDeclOnDevice[deviceId].push_back(fn);
        continue;
    }
  ...
  if (stmt.num_devices == 0) {
    stmt.num_devices = 1;
    stmt.activeDevices.push_back(0);
  } else {
      bool found = false;
      for (AST::FunctionDeclaration *stepFunc : stmt.stepFuncDecls) {
          ...
          std::ostringstream s;
          s << "coexecution_merge_" << type;
          auto it = funcDecls.find(s.str());
          if (it != funcDecls.end()) {
              found = true;
          }
      }
      if (!found) {
          err << "A merge function is required for co-execution" << stmt.loc;
          return;
      }
  }
  script.simStmt = &stmt;
